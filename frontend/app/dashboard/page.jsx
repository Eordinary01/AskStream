...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      Promise.all([fetchUserData(token), fetchOrganizations(token)]).finally(() =>
        setLoading(false)
      );
    }
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <label className="flex items-center gap-3">
                      ...
                      <span className="text-gray-800">
                        Allow only one question per user
                      </span>
                    </label>
                    <div>
                      <label className="block mb-1 text-gray-800">
                        Cooldown time (seconds, disable if above checked)
                      </label>
                      <input
                        type="number"
                        min="0"
